...
$(document).ready(function() {
  ...
  onResults(function(versionCode, expStartTimestamp, profile, results) {
          ...
          for (var k = 0; k < questions.length; k++) {
            ...
            if ('answer' in question) { // si la reponse est dans la question ...
              if ('sliders' in question['answer']) {
                var question_string = Object.keys(question['answer']['sliders'])[0] // dictionnary has a single key
                var answer = question['answer']['sliders'][question_string]
                if (question_name === "sleep") {
                  answer = Math.floor(answer * 16 / 100);
                }
                dict[question_name] = answer
              }
            } else {
              isValid = false;
            } // skipping questions without answers
          }
          if (isValid) {
            morning_q.push(dict);
          }
    ...
    if (morning_q.length > 0) {
      ...
      morning_q.sort(function(a, b) {
          return a.day - b.day
        }) // sorting increasing date
      var exp_days = []
      var exp_days_string = []
    ...
    function scores(array) {
      ...
      for (var i = 0; i < array.length; i++) {
        var item = array[i];
        var type = item.type;
        if (item.name === "MAAS" && item.index < 16) {
          score[0].push(100 - item.value);
        }
        if (item.name === "SODAS") {
          score[1].push(item.value);
        }
        if (item.name === "RR") {
          var value = item.value;
          if ($.inArray(item.index, [6, 9, 10, 13, 14, 17, 20, 24])) {
    ...
    var ws_bar = {
      ...
      display: function() {
        ...
        var x = d3.scale.ordinal()
          ...
          ]), // extend graph by 20% to allow for legend on top
      ...
      display: function() {
        ...
        vis.selectAll("dot")
          ...
          .attr("ry", function(d, i) {
            return 3.5 + d.dreams * 0.1;
          })
          .attr("rx", function(d, i) {
            return 3.5 + d.dreams * 0.1;
          })
          .attr("fill", function(d, i) {
            var col = (Math.floor(d.valence * 2.55)).toString();
            return "rgba(" + col + "," + col + "," + col + ",0.8)";
          })
          .attr("cx", function(d, i) {
            return x(d.date);
          })
    ...
    // Add share results button if provided by the app
